...
class QDDETR(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
        ...
        # # self.saliency_proj_out = nn.Linear(transformer.d_model, transformer.d_model)
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_aud=None, src_aud_mask=None):
        ...
        # # 使用一下置信度来优化相关性分数：span
        # # 使用检索出来的置信度最高的Moment的全局信息和所有clip进行计算匹配分数，然后以此优化高光分数
        # #  [[0.3870, 0.1250],
        # #  [0.4347, 0.0448],
        # #  [0.6679, 0.0872],
        # #  [0.4352, 0.3251],
        ...
        sorted_indices_max  = sorted_indices[:,:1] # 取出第一列的值
        # ensor([0.6752, 0.6207, 0.6152, 0.4388, 0.4788, 0.6530, 0.6955, 0.5728, 0.6711,                                                                | 0/226 [00:00<?, ?it/s]
        # 0.5436], device='cuda:0', grad_fn=<SelectBackward0>) tensor([6, 0, 8, 5, 1, 2, 7, 9, 4, 3], device='cuda:0')

        # print(scores[0], sorted_indices[0])
        # print(sorted_indices_max.shape) # orch.Size([32, 1])
        pred_spans = out['pred_spans']
        spans = span_cxw_to_xx(pred_spans) * (video_length * self.clip_len)  # 进行偏移， meta["duration"]
        spans = torch.floor(spans / self.clip_len) # 用于偏移到video_length，便于处理memory
        # cur_ranked_preds = torch.cat([spans, scores.unsqueeze(-1)], dim=-1)
        # print(spans.shape) # orch.Size([32, 10, 2])
        selected_values_max = spans[torch.arange(spans.size(0)).unsqueeze(1), sorted_indices_max].squeeze(1)
        # print(selected_values_max, cur_ranked_preds)
        ...
        for i in range(b):
            ...
            # 获取当前样本的起始时间戳和结束时间戳之间的切片
            sliced_sample = src_vid_ed[i, start_time:end_time + 1, :] # 这里改为映射后的值 / memory
        ...
        mask = (sliced_features != 0.0)  # 生成填充掩码矩阵

        # 均值
        # sliced_features_global = sliced_features.mean(1)
        # # print(sliced_features_global.shape) # orch.Size([32, 256]) 
        # sliced_features_global_expanded = sliced_features_global.unsqueeze(1)  # shape: (32, 1, 256)


        # GRU学习
        sliced_features_global_expanded = self.gru_extractor(sliced_features).unsqueeze(1) # orch.Size([32, 256])
        # print(sliced_features_global_expanded.shape)
        ...
        # weight = process(spans_score, 5, video_length)
        # print(weight[0])
        # memory = memory*(torch.softmax(weight, -1).cuda()).unsqueeze(-1)  +  memory
        memory = memory*weight.unsqueeze(-1)  +  memory # Raw
        # 用于映射最终分数
        saliency_scores = (torch.sum(self.saliency_proj1(memory), dim=-1) / np.sqrt(self.hidden_dim)) # (bs, video_length)
        # saliency_scores = saliency_scores + cosine_similarity_matrix
        # saliency_scores = saliency_scores*(torch.softmax(weight, -1).cuda())  + saliency_scores
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, log=True):
        ...
        # 使用focal loss代替交叉熵损失 ValueError: Target size (torch.Size([32, 10])) must be the same as input size (torch.Size([32, 2, 10]))
    ...
    def loss_saliency(self, outputs, targets, indices, log=True):
        ...
        saliency_scores = vid_token_mask * saliency_scores + (1. - vid_token_mask) * -1e+3 # 防止log无了
        ...
        loss_saliency = loss_saliency + loss_rank_contrastive # 猜测是这里的原因
